...
function extractBracketedFields(text: string): {
  cleanText: string;
 ...
 */
function extractUnbracketedFields(text: string): {
  ...
  for (const child of tree.children) {
    ...
    if (child.type === 'heading') {
      ...
      if (stack.length > 0) {
        ...
        if (depth > topDepth + 1) {
          const rawTitle = mdastToString(heading);
          throw new Error(`Heading level skipped: jumped from H${topDepth} to H${depth} at "${rawTitle}"`);
        }
      }
